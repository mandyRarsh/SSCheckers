import { Square, Color, SquareLocation } from "./Square";
...
export class CheckerPiece {
    ...
    moveTo(square: Square, player: Player, opposingPlayer: Player, jumpedChecker?: CheckerPiece) {
        if (this.canMoveTo(square, !!jumpedChecker)) {
            ...
            player.moveNumber++;
            if (jumpedChecker) {
                jumpedChecker.jump(player, opposingPlayer);
                jumpedChecker.position.occupiedBy = null;
            }
            this.tryKing(square);
    ...
    jump(player: Player, opposingPlayer: Player) {
        opposingPlayer.removeChecker(this);
        this._isJumped = true;
    }
